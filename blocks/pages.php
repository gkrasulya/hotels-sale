	...
	if (mysql_num_rows($result) > $x)
		{
			...
			if ($prev_2page != 0 && $prev_2page != -1)
				{ echo "<a href='?search&countries[]=".implode($countries, ',')."&max_price=".$max_price."&min_price=".$min_price."&page=".$prev_2page."'> ".$prev_2page." </a>"; }
						
			if ($prev_page > 0)	
				{ echo "<a href='?search&countries[]=".implode($countries, ',')."&max_price=".$max_price."&min_price=".$min_price."&page=".$prev_page."'> ".$prev_page." </a>"; }
				
			echo "<span>".$page."</span>";
				
			if ($next_page < $max_page + 1)	
				{ echo "<a href='?search=&countries[]=".implode($countries, ',')."&max_price=".$max_price."&min_price=".$min_price."&page=".$next_page."'> ".$next_page." </a>"; }
			
			if ($next_2page != $max_page + 1 && $next_2page != $max_page + 2)
				{ echo "<a href='?search=&countries[]=".implode($countries, ',')."&max_price=".$max_price."&min_price=".$min_price."&page=".$next_2page."'> ".$next_2page." </a>"; }
			
			if ($next_2page < $max_page)
				{ echo "<a href='?search=&countries=".implode($countries, ',')."&max_price=".$max_price."&min_price=".$min_price."&page=".$max_page."'> >> </a>"; }
