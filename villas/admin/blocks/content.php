									...
									if (isset($do))
										{
											...
											<form method='post' id='form' action='?t=main&do'>
												...
												<textarea name='meta_k'><?= $myrow['meta_k'] ?></textarea>

												<label>Краткое описание (для поисковиков)</label>
												<textarea name='meta_d'><?= $myrow['meta_d'] ?></textarea>
							...
							if ($t == "random")
								{
									...
									echo "<h2>Âûâîä âèëëû â ïðàâîé ÷àñòè</h2>";
									...
									if (isset($do))
										{
											...
											<form method='post' id='form' action='?t=random&do'>
												<label>Âûáåðèòå îäíó èëè íåñêîëüêî âèëë</label>
											...
											echo "
												...
												Ïîêàçûâàòü â ñëó÷àéíîì ïîðÿäêå
									...
									if (isset($do))
										{
											...
											echo "
											<br>
											<p>Òåêóùèé ëîãèí: <strong>".$myrow['login']."</strong></p>
											<p>è ïàðîëü: <strong>".$myrow['password']."</strong></p>
											<form method='post' id='form' action='?t=password&do'>
												<label>Íîâûé ëîãèí</label>
												<input type='text' name='login' value='".$myrow['login']."'>
												<label>Íîâûé ïàðîëü</label>
												<input type='text' name='password' value='".$myrow['password']."'>
												<label>E-mail (êîòîðûé îòîáðàæàåòñÿ íà ñàéòå)</label>
											...
											if (!isset($n))
												{
													...
													echo "<h2>Ðåäàêòèðîâàíèå ñòðàíû</h2>
													<form method='post' id='form'>
														<label>Âûáåðèòå ñòðàíó</label><br>";
									...
									if ($a == "delete")
										{
											...
											echo "<br>
												<label>Íàæìèòå íà ñòðàíó, ÷òîáû åå óäàëèòü</label><br><br>";
											...
											if (!isset($n))
												{
													...
													echo "<h2>Ðåäàêòèðîâàíèå ðåãèîíà</h2>
													<form method='post' id='form'>
														<label>Âûáåðèòå ðåãèîí</label><br>";
									...
									if ($a == "delete")
										{
											...
											echo "<br>
												<label>Íàæìèòå íà ðåãèîí, ÷òîáû åãî óäàëèòü</label><br><br>";
									...
									if ($a == "add")
										{
											...
											<form method='post' id='form' enctype='multipart/form-data' action='?t=hotel&a=add&do'>
												...
												<label>Öåíà (áåç ïðîáåëîâ)</label>
												<input type='text' name='price_s'>
												<label>Êðàòêîå îïèñàíèå</label>
												<textarea name='desc'></textarea>
												<label>Ïîäðîáíîå îïèñàíèå</label>
												<textarea name='text'></textarea>
												<label>Ôîòîãðàôèÿ</label>
												...
												$result = mysql_query("SELECT * FROM countries",$db);
												$myrow = mysql_fetch_array($result);
												echo "<option value='0'> Áåç ðåãèîíà </option>";
													do
														{
											...
											if (!isset($n))
												{
													...
													echo "<h2>Ðåäàêòèðîâàíèå âèëëû</h2>
													<form method='post' id='form'>
														<label>Âûáåðèòå ãîñòèíèöó</label><br>";
											...
											else
												{
													...
													echo "<h2>Ðåäàêòèðîâàíèå ãîñòèíèöû</h2>";
									...
									if ($a == "delete")
										{
											...
											echo "<br>
												<label>Íàæìèòå íà ãîñòèíèöó, ÷òîáû åå óäàëèòü</label><br><br>";
...
Öåíà: ".$hotels['price'].", Êîìíàò: ".$hotels['rooms']." \n
".$hotels['descr']."\n
ïîäðîáíåå: http://hotels-sale.ru/?h=".$hotels['id']."\n\n\n";
											...
											do
												{
													$to = $myrow['email'];
													$subject = "Hotels-sale.ru, Villas: Íîâûå ïðåäëîæåíèÿ";
													$body = "Äîáðûé äåíü, ".$myrow['name']."! Íîâûå ïðåäëîæåíèÿ: \n\n
".$new."Ïîñìîòðåòü íîâûå ïðåäëîæåíèÿ Âû ìîæåòå çäåñü: http://hotels-sale.ru/villas/?new";
											...
											if ($mail)
												{
													echo "<h2>Ïèñüìà îòïðàâëåíû!</h2>";
												}
											else
												{
													echo "<h2>×òî-òî ïîøëî íå òàê!</h2>";
												}
									...
									if (isset($_GET['delete']))
										{
											...
											echo "<br>
												<label>Íàæìèòå íà àäðåñàòà, ÷òîáû åãî óäàëèòü</label><br><br>";
									...
									if (!isset($_GET['act']) && !isset($_GET['delete']))
										{
											echo "<h2>Ðàññûëêà</h2>
												<a href='?t=mail&act'>Ðàçîñëàòü</a><br/><br/>
											...
											if (!isset($do))
												{
													...
													echo "
														<form action='?t=photos&a=add&do' method='post' enctype='multipart/form-data'>
															<p>
																<label>Ôîòîãðàôèÿ:</label><br/>
																<input type='file' name='foto' />
															</p>
															<p>
																<label>Ãîñòèíèöà</label><br/>
									...
									if ($a == 'delete')
										{
											if (isset($do))
												...
												{
													echo "<h2>Âûáåðèòå ôîòî, êîòîðîå õîòèòå óäàëèòü</h2>";
													$result = mysql_query("SELECT id,title,number FROM hotels ORDER BY country",$db);
													$hotels = mysql_fetch_array($result);
													do
														{
