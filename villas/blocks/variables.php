...
if (isset($_GET['about'])) // About company
	{
		$about = $_GET['about'];
		$title = "- О компании";
		$about_link = " style='text-decoration:none;background-color:#FFCC99;color:#9a2a00;border-right:5px solid #ac5c10;cursor:default;'";
	}
if (isset($_GET['new'])) // New hotels
	{
		$new = $_GET['new'];
		$title = "- Новый предложения";
		$new_link = " style='text-decoration:none;background-color:#FFCC99;color:#9a2a00;border-right:5px solid #ac5c10;cursor:default;'";
	}
if (isset($_GET['getmail']))
	{
		$getmail = $_GET['getmail'];
	}
if (!isset($h) && !isset($r) && !isset($qwe) && !isset($about) && !isset($new)) // Main page
	{
		$main_link = " style='text-decoration:none;background-color:#FFCC99;color:#9a2a00;border-right:5px solid #ac5c10; cursor:default;'";
	}
...
$s = "ORDER BY id DESC"; // Default sorting
$sort_by_town = ""; // Style
$sort_by_price = ""; // Style
if (isset($_GET['s']))  // Sorting
	{
		$s = $_GET['s'];
		$s == "town" ? $s = "ORDER BY town" : $s = "ORDER BY price_s";
		$s == "ORDER BY town" ? $sort_by_town = "style='background:#eaeaff; text-decoration:none;'" : $sort_by_price = "style='background:#eaeaff; text-decoration:none;'";
	}
