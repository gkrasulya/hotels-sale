...
session_start();
if($_GET['action']=="move_trash" AND $_GET['id'] AND $_SESSION['owner_status']=="this_admin") {
...
$trash=fopen("data/trash.dat","a");
fwrite($trash, "$base[$i]"); // иначе пишем данные в корзину
fclose($trash);
...
// Если нужно, отправим письмо, что ссылка удалена
// -----------------------------------------------------------------------------

// читаем нужное письмо
$lock = fopen("data/base.lck","a");
if(flock($lock, LOCK_EX)) {
$subject_2 = file("letters/s2.txt");
$message_2 = file("letters/m2.txt");
$letter_swith_2 = file("letters/w2.txt");
flock($lock, LOCK_UN);
fclose($lock);
} else {
$_SESSION['error'] = "Не могу заблокировать служебный файл data/base.lck !!!";
...
$subject_2 = implode("",$subject_2);
$message_2 = implode("",$message_2);
$letter_swith_2 = implode("",$letter_swith_2);

if(trim($letter_swith_2)=="on") {
$message_2 = str_replace(NICK_NAME, $nick, "$message_2");
$message_2 = str_replace(LINK_PLACE, $link_place, "$message_2");
$message_2 = str_replace(BACK_LINK, $urlink, "$message_2");
$message_2 = str_replace(OUT_LINKS, $check_out_links, "$message_2");
$message_2 = str_replace(SITE_CY, $check_cy, "$message_2");
$message_2 = str_replace(PAGE_PR, $check_pr, "$message_2");
$message_2 = str_replace(IMP_MYHOME, $_SESSION[imp_myhome], "$message_2");
$message_2 = str_replace(ADM_E_MAIL, $_SESSION[adm_e_mail], "$message_2");
$message_2 = str_replace(ADM_COMMENT, $admin_comment, "$message_2");

switch($_SESSION['adm_out_mail']) {
case "koi8-r":
$subject_2 = convert_cyr_string($subject_2, "w", "k");
$message_2 = convert_cyr_string($message_2, "w", "k");
$charset = "koi8-r";
break;
case "iso-8859":
$subject_2 = convert_cyr_string($subject_2, "w", "i");
$message_2 = convert_cyr_string($message_2, "w", "i");
$charset = "iso-8859-1";
break;
default:
...
mail($mail, "$subject_2", "$message_2", "$headmail");
}
