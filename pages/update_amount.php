...
<? if ($_SERVER['REQUEST_METHOD'] == 'GET' or ! empty($errors)): ?>
	...
	<form action="." method="POST" class="login-form">
		<div class="form-row">
			<label for="sum">Сумма (в рублях)</label><br />
			<?
			$summ_value = $periods[0]['price'];
			if (isset($_POST['sum'])) {
				$summ_value = $_POST['sum'];
			} elseif (isset($_GET['sum'])) {
				$summ_value = $_GET['sum'];
			}
			?>
			<input type="text" class="small" name="summ" id="summ" value="<?= $summ_value ?>" maxlength="6" />
			<span id="inEuro" class="hint"></span>
			<script type="text/javascript">
				;(function() {
					var euroRate = <?= $euro_rate ?>,
						$summ = $('#summ'),
						$inEuro = $('#inEuro'),
						keyupHandler = function() {
							var inEuro = (parseInt($summ.val().replace(/\s/, '')) / euroRate).toFixed(2);
							if (inEuro == 0) inEuro = 1;
							$text = isNaN(inEuro) ? '' : '(€' + inEuro + ' )';
							$inEuro.text($text);
						};

					keyupHandler();
					$summ.keyup(keyupHandler);
				})();
			</script>
		</div>
	...
	<script type="text/javascript">
		(function() {
			...
			$summ.keyup(function(event) {
				if ($summ.val().trim() != '' && paymentSelected) $submit.attr('disabled', false);
			...
			$paymentSystems.each(function() {
				...
				$self.click(function() {
					...
					if (! paymentSelected) {
						...
						if ($summ.val().trim() != '') $submit.attr('disabled', false);
					}
