									...
									if (isset($do))
										{
											...
											<form method='post' id='form' action='?t=random&do'>
												<label>Выберите одну или несколько гостиниц</label>
											...
											if (isset($do))
												{
													...
													(title,town,text,rooms,price,number,region,country,descr,foto,price_s,tosend) VALUES ('$name','$town','$text','$rooms','$price','$number','$region','$country','$desc','$foto_id','$price_s','1')",$db);
													...
													if (isset($do))
														{
															...
															$result = mysql_query("UPDATE hotels SET title='$name' ,town='$town',text='$text',rooms='$rooms',price='$price',number='$number',region='$region',descr='$desc',country='$country',price_s='$price_s'".$foto_id." WHERE id='$n'",$db);
...
$new = '';
											...
											do
												{
if ($hotels['rooms'] != '0') { $rooms = "Цена: ".$hotels['price'].", Номеров: ".$hotels['rooms']." \n"; } else { $rooms = "Цена: ".$hotels['price']."\n"; }
...
".$new."Посмотреть новые предложения Вы можете здесь: http://hotels-sale.ru/?new";
